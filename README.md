...
- `key='v181_fullauto_project'` 중복 오류 수정
- V18.1 통합운영센터 상단/하단 중복 표시 제거
- 원클릭 자동반영 센터는 1번만 유지
- 사진등록 유지
- 기존 개별 메뉴 유지
...
- 원클릭 자동반영 센터를 기본 화면 중심으로 정리
- 상세 정보는 화살표 expander로 숨김
- 프로젝트 정보 / 커밋 메시지 / 업로드 상세결과 / 오류 원본 / 사진 상세결과 / 사용법 접기 처리
- 사진등록 유지
- ZIP/app.py 자동반영 유지
- 기존 메뉴 유지
...
추가:
- 패치/개선/오류수정/사진분석/기능추가/UI정리 명령사항 자동처리 센터
- 명령사항도 ZIP 업로드와 동일한 순서로 처리
  1. 프로젝트 선택
  2. 명령 입력
  3. 첨부파일/사진 등록
  4. 보관소 저장
  5. GitHub maru_commands 폴더 기록
  6. 풀자동화 실행
  7. GitHub 자동반영
- 기존 ZIP/app.py 원클릭 자동반영 유지
- 기존 사진등록 유지
- 기존 메뉴 유지
...
오류 수정:
- `name 'maru_v195_command_workflow_center' is not defined`
- 함수 정의보다 호출이 먼저 실행되던 순서 문제 수정

숨김 정리:
- 패치·개선·명령사항 자동처리 센터를 기본 접힘 처리
- 명령 입력 영역도 기본 접힘 처리
- 오류 원본은 화살표로 접기 처리
...
## V20 전체 메뉴·기능 검사 안정화판

이번 버전은 기능을 더 붙이는 게 아니라 전체 메뉴와 핵심 기능을 검사하는 센터를 추가했습니다.
